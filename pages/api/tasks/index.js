  ...
  switch (related_to) {
    case "invoicesGet":
      assignid = await executeQuery(
        "SELECT staffid as id FROM tblstaff WHERE firstname = '" + assign + "'",
        `odoads_${databse}`
      );
      break;
    case "customers":
    ...
    case "tradeLandload":
      assignid = await executeQuery(
        "SELECT id FROM tbllandlords WHERE name = '" + assign + "'",
        `odoads_${databse}`
      );
      break;
    default:
