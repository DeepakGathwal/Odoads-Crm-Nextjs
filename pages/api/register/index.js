...
export const phoneOTp = catchError(async (req, res) => {
  ...
  const data = await executeQuery("SELECT id FROM tblcompanies WHERE contact_phone = " + phone + " && code IS NOT NULL", "odoads_tblcompanies"); console.log(data); if (data.length == 0) {
    const del = await executeQuery("DELETE  FROM tblcompanies WHERE contact_phone = " + phone + "", "odoads_tblcompanies")
    if (del) {
      let otp = Math.floor(100000 + Math.random() * 900000); const sql = await executeQuery("INSERT into tblcompanies SET contact_phone = " + phone + ", contact_otp = " + otp + " ", "odoads_tblcompanies");
      if (sql) {
  ...
  promises.push(new Promise(async (resolve, reject) => {
    ...
    if (already.length == 0) {
      const newpassword = bcryptjs.hashSync(String(contact_password), 8); //, , 1, CURRENT_TIMESTAMP, true, self                                                                                                                                            
            ...
            const sql = await executeQuery("UPDATE tblcompanies SET name = '" + code +"', code = '" + code + "', contact_email = '" +          contact_email +          "', contact_firstname = '" +          newName +          "', contact_password = '" +          newpassword +          "',contact_gstin = '" +          gstin +          "',  status = 1, created = CURRENT_TIMESTAMP, register = true, add_by = 'self' WHERE contact_phone = '" +          contact_phone +          "'",        "odoads_tblcompanies"      );      
            resolve(sql)      
            if (sql) {        
...
export const addUser = catchError(async (req, res) => {
  ...
  if (userData) {
    ...
    const data = await executeQuery("INSERT into tblstaff SET email = '" + userData[0].contact_email + "', firstname = '" + userData[0].name + "', phonenumber = " + userData[0].contact_phone + ", password ='" + userData[0].contact_password + "', datecreated = CURRENT_TIMESTAMP", `odoads_${code}`)
...
//   switch (method) {
//     case "POST":
//       registerComapny(req, res);
//       break;
//     case 'GET': await addUser(req,res);
//     break;
//     case "PATCH":
//      await phoneOTp(req, res);
//       break;
//     case "PUT":
//       verifyOtp(req, res);
//       break;
...
//     "SELECT id FROM tblcompanies WHERE contact_phone = " + phone + "",
//     "odoads_tblcompanies"
//   );
...
//   const data = await executeQuery(
//     "SELECT id FROM tblcompanies WHERE contact_otp = " + otp + "",
//     "odoads_tblcompanies"
//   );
//   if (data.length != 0) {
//     return res.status(200).json({ message: "Your can countinue" });
...
// export const registerComapny = catchError(async (req, res) => {
//   const {value} = req.body
//   const { gstin, company_name, contact_email, contact_phone, contact_password } =
//     value;
//     const promises = []
//   const code = company_name.replace(" ", "_");
//   promises.push(new Promise (async(resolve, reject) =>{
//     const already =await  executeQuery("SELECT id FROM tblcompanies WHERE code = '"+code+"'", "odoads_tblcompanies")
//     resolve(resolve)
//     if(already.length == 0){
//       const newpassword = bcryptjs.hashSync(String(contact_password), 8); //, , 1, CURRENT_TIMESTAMP, true, self                                                                                                                                                // name,               code,        contact_email,     contact_gstin, pan, status, created, register, addedby
//       const sql = await executeQuery(
//         "UPDATE tblcompanies SET name = '" +
//           company_name +
//           "', code = '" +
//           code +
//           "', contact_email = '" +
...
//           "',contact_gstin = '" +
//           gstin +
//           "',  status = 1, created = CURRENT_TIMESTAMP, register = true, add_by = 'self' WHERE contact_phone = '" +
//           contact_phone +
//           "'",
//         "odoads_tblcompanies"
//       );
...
//   const data = await executeQuery("INSERT into tblstaff SET email = '"+userData[0].contact_email+"', firstname = '"+userData[0].name+"', phonenumber = "+userData[0].contact_phone+", password ='"+userData[0].contact_password+"'",`odoads_${code}`)
// if(data){
//   const getStaff = await executeQuery("SELECT staffid from tblstaff WHERE staffid = (SELECT Max(staffid) FROM tblstaff)",`odoads_${code}`)
//  if(getStaff){
//   const id = `${getStaff[0].staffid}&${code}` 
//   res.setHeader("Set-Cookie",cookie.serialize(String(`odoads_goh`),{expires: Date.now()}))
//   token(id, 200, res);
//  }else{
//   return res.status(200).json({message : "Something wrong"})
