...
export default async function handler(req, res){
   const {method} = req
    switch(method){
        case 'POST': changePassword(req,res)
        break;
        case 'PUT':
        ...
        break;
        case 'GET': await verifyToken(req, res);  await getUser(req,res);
        break;
         default:
...
export  const loginEmail = catchError(async(req, res) =>{
    ...
    const data = await executeQuery("SELECT code FROM tblcompanies WHERE name = '"+company+"' ","odoads_tblcompanies") 
  
    if(data.length == 1){
      const userid = data[0].code
      const stafflogin = await executeQuery("SELECT password, staffid From tblstaff WHERE email = '"+email+"'", `odoads_${userid}`);
...
export const getUser = catchError(async(req,res,next) =>{
   const {code, userid} = req.id
    const data = await executeQuery("SELECT staffid as id, firstname, facebook,linkedin,phonenumber,skype, profile_image FROM tblstaff WHERE staffid = "+userid+" ",`odoads_${code}`) 
    if(data.length == 1){
...
export const changePassword = catchError(async (req, res) => {
  ...
  if(password !== undefined && confirmPassword !== undefined && company !== undefined){
    ...
    const finalPassword = bcrypt.hashSync(String(password), 8)
      if (!finalPassword) {
          ...
          const sql = await executeQuery("SELECT code from  tblcompanies WHERE  name='" + company + "'", "odoads_tblcompanies");
              if (sql) {
                  const Companycode = sql[0].code
                const updatePassword = await executeQuery("Update tblstaff SET password = '"+finalPassword+"', last_password_change = CURRENT_TIMESTAMP WHERE phonenumber = "+contactPhone+"", `odoads_${Companycode}`)
               if(updatePassword){
                const staffid = await executeQuery("SELECT staffid FROM tblstaff  WHERE phonenumber = "+contactPhone+"", `odoads_${Companycode}`)
...
export const logOut = catchError(async(req,res,next) =>{
  ...
  if(userid == id){
    const updateDatabse = await executeQuery("Update tblstaff SET last_activity = CURRENT_TIMESTAMP, last_login = CURRENT_TIMESTAMP WHERE staffid = "+id+" ",`odoads_${code}` )
   if(updateDatabse){
     const option = {
       path: '/',
       httpOnly:true,
   ...
   return res.status(200).setHeader("Set-Cookie",cookie.serialize(String(logoutData), "thisismysecretejsonWebToken", option)).json({
       success: true,
