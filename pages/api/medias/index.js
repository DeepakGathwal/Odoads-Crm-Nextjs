...
export default async function handler(req, res){
   const method = req.method
    switch(method){
        case 'POST': await verifyToken(req,res); await addMedia(req,res);
        break;
        case 'GET': await verifyToken(req,res); await allMedia(req,res);
        break;
        case 'PATCH': await subcategoryis(req,res);
        break;
...
if(value ){
const {name,total,mediaPrice,city, district,ftf, height,width,location, category,subcategory,illumination, measuring,geo_location,available_date} = value
const data = await executeQuery("SELECT id FROM tblmedia_deails WHERE medianame = '"+name+"'", `odoads_${databse}`)
if(data.length == 1){
...
const ann = "INSERT INTO tblmedia_deails (category_id, sub_category_id, medianame, totalno, code, price, pricetype, thumbnail, state, city, district, width,widthunit, height, heightunit, illumination, ftf, areadescription, location, geolocation,  available_date,syncstatus, created_date, owner) VALUES ('"+$tblnm+"',"+Subcategory_id+", '"+name+"', "+total+", '"+Mediacode+"', "+mediaPrice+", 'Month', '"+mediaImage+"',"+state+","+cityid+", '"+district+"',"+width+",'"+measuring+"', "+height+",'"+measuring+"', '"+illumination+"','"+ftf+"','"+location+"','"+location+"', '"+geo_location+"','"+availabledate+"','added', CURRENT_TIMESTAMP,"+userId+" )"
...
if(state){      
    const {id, medianame,totalno,price,city, district,ftf, height,width,location, category,subcategory,illumination, heightunit,geolocation,available_date} = state
...
const result = await executeQuery("SELECT id, name FROM tblmedia_categories WHERE name='"+subcategory+"' UNION  SELECT id, state_id FROM tblcities WHERE name='"+city+"'", "odoads_general_tbl")
if(result){
...
const newMedia = "UPDATE tblmedia_deails SET totalno = "+totalno+",sub_category_id = "+Subcategory_id+", medianame =  '"+medianame+"', "+updateCtegoryfield+" price =  "+price+",  thumbnail = '"+price+"', state = "+state+", city = "+cityid+", district = '"+district+"', width = "+width+",widthunit = '"+heightunit+"', height =  "+height+", heightunit = '"+heightunit+"', illumination =  '"+illumination+"', ftf = '"+ftf+"', areadescription = '"+location+"', location = '"+location+"', geolocation =  '"+geolocation+"',  available_date = '"+available_date+"',syncstatus = 'added', modify_date = CURRENT_TIMESTAMP WHERE id = "+id+""
...
const subCate = await executeQuery("SELECT name  from tblmedia_categories WHERE p_id = "+$tblnm+"","odoads_general_tbl")
if(subCate){
