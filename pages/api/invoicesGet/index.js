...
export default async function handler(req, res){
    const method = req.method
     switch(method){
         case 'POST': forgetOtp(req,res)
         break;
         case 'PATCH': matchOtp(req,res)
         break;
      
         case 'PUT': await allTaxes(req,res)
         break;
...
const phoneNymber =  await executeQuery("SELECT staffid From tblstaff WHERE phonenumber = "+contactPhone+"", `odoads_${companycode}`)
...
if(phoneNymber.length == 1){
          ...
          async function (error, response, body) {
            if (error) {
              ...
              const sql = await executeQuery(
                "Update tblstaff SET new_pass_key = " +otp + ", new_pass_key_requested = CURRENT_TIMESTAMP   WHERE phonenumber = "+contactPhone+"",`odoads_${companycode}`
              );
              if (sql) {
                return res
                  .status(200)
 ...
 if(data.length == 1){
const companycode = data[0].code
const phoneNymber =  await executeQuery("SELECT staffid From tblstaff WHERE new_pass_key = "+contactOTP+"", `odoads_${companycode}`)
if(phoneNymber.length == 1){
    return res.status(200).json({success: true, message: "Your can countinue" });
  } else {
 ...
 export const allTaxes = catchError(async(req,res) =>{
  const taxes  = await executeQuery("SELECT taxname FROM tblitemstax", "odoads_general_tbl")
